...
class ShoppingListCard extends HTMLElement {
  ...
  _loadSettings() {
    ...
    if (this._config) {
      ...
      if (this._config.hide_section_headers  !== undefined) defaults.hideSectionHeaders  = this._config.hide_section_headers;
    }

    // Layer 2: localStorage cache (wins over config only if it exists,
    // keeps things instant while the page is still loaded)
   ...
   */
  _saveSettings() {
    // Mirror into config object so the GUI editor stays in sync
    // if it re-reads config later, but do NOT fire config-changed
    // here — that would trigger setConfig() which resets
    // this._settings mid-flow and breaks the cog modal.
    this._config.haptics              = this._settings.haptics;
    this._config.products_per_row     = this._settings.productsPerRow;
    this._config.layout               = this._settings.layout;
    this._config.hide_completed       = !!this._settings.hideCompleted;
    this._config.hide_section_headers = !!this._settings.hideSectionHeaders;

    // Persist to localStorage (fast, survives page reload)
    const key = this._settingsKey || 'shopping_list_settings_default';
  ...
  _updateContent() {
    ...
    if (inactiveProducts.length > 0 && !this._searchQuery && !this._config.hide_completed) {
      html += `
  ...
  _renderProductsByCategory(products, isRecentlyUsed) {
    ...
    return CATEGORIES
      ...
      .map(cat => {
        ...
        const headerHtml = this._config.hide_section_headers ? '' : `
...
class ShoppingListCardEditor extends HTMLElement {
  ...
  set hass(hass) {
    this._hass = hass;
    // HA often sets hass AFTER setConfig — re-render so ha-select
    // and ha-switch components get their values properly initialised.
  ...
  _render() {
    ...
    const hideSectionHdrs = !!this._config.hide_section_headers;
    ...
    this.shadowRoot.innerHTML = `
      ...
      <!-- Toggle: Hide section headers -->
      <div class="toggle-row">
        <div class="toggle-text">
          <span class="toggle-label">Hide section headers</span>
          <span class="toggle-hint">Removes the category headers when sorted by category.</span>
        </div>
    ...
    hideHeadersEl.checked  = hideSectionHdrs;
    ...
    const updateConfig = () => {
      this._config = {
        ...
        hide_section_headers: hideHeadersEl.checked
      };
  ...
  _toYaml() {
    ...
    if (this._config.hide_section_headers) lines.push('hide_section_headers: true');
    return lines.join('\n');
  }
}
